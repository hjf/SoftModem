...
{
	// Calculate the number of cycles for the desired tone duration
	unsigned long toneDuration = 150; // Duration in milliseconds
	unsigned long endTime = millis() + toneDuration;
	uint8_t cnt = (uint8_t)(LOW_FREQ_CNT);
	uint8_t tcnt = (uint8_t)(TCNT_LOW_FREQ);
	uint8_t tcnt2 = (uint8_t)(TCNT_LOW_FREQ / 2);

	while (millis() < endTime)
	{
		uint8_t cycleCnt = cnt;
		do
		{
			cycleCnt--;
			OCR2B += tcnt;
